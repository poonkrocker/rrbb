...
<body>
 ...
 <!-- Business Hours Management -->
            ...
            <!-- Bulk Price Editor -->
            <details>
                <summary>Actualización Masiva</summary>
                <div class="editor-form">
                    <h3>Editar precios de todos los productos</h3>
                    <p class="note" style="margin-bottom:12px;">
                        Modificá los precios directamente en la tabla y guardá con un solo clic. Dejá el precio secundario en blanco para no modificarlo.<br>
                        <strong>👁</strong> = visible en carta &nbsp;|&nbsp; <strong>🚫</strong> = oculto en carta (clic para alternar, se guarda al instante) &nbsp;|&nbsp;
                        <strong>⠿</strong> = arrastrá para reordenar dentro de la categoría &nbsp;|&nbsp; <strong>✕</strong> = eliminar producto
                    </p>
                    <form method="POST" id="bulk-price-form">
                        <input type="hidden" name="csrf_token" value="<?php echo $_SESSION['csrf_token']; ?>">
                        ...
                        <table class="hours-table" style="width:100%;min-width:500px;margin-bottom:8px;">
                            ...
                            <tbody class="bulk-sortable" data-category="<?php echo $cat['id']; ?>">
                                <?php foreach ($items_by_cat[$cat['id']] as $item): ?>
                                <tr data-id="<?php echo $item['id']; ?>" style="<?php echo !$item['is_visible'] ? 'opacity:0.5;' : ''; ?>">
                                    <td style="cursor:grab;color:#aaa;font-size:1.1rem;text-align:center;user-select:none;" class="bulk-drag-handle" title="Arrastrá para reordenar">⠿</td>
                                    <td><?php echo htmlspecialchars($item['name']); ?></td>
                                    <td>
                                        <input type="number" step="0.01" min="0.01" required
                                            name="prices[<?php echo $item['id']; ?>]"
                                            value="<?php echo htmlspecialchars($item['price']); ?>"
                                            style="width:110px;max-width:110px;padding:6px;border:1px solid #ccc;border-radius:5px;text-align:right;">
                                    </td>
                                    <td>
                                        <input type="number" step="0.01" min="0.01"
                                            name="secondary_prices[<?php echo $item['id']; ?>]"
                                            value="<?php echo htmlspecialchars($item['secondary_price'] ?? ''); ?>"
                                            placeholder="—"
                                            style="width:110px;max-width:110px;padding:6px;border:1px solid #ccc;border-radius:5px;text-align:right;">
                                    </td>
                                    <td style="text-align:center;">
                                        <button type="button"
                                            ...
                                            title="<?php echo $item['is_visible'] ? 'Visible — clic para ocultar' : 'Oculto — clic para mostrar'; ?>"
                                            style="background:none;border:none;cursor:pointer;font-size:1.2rem;padding:2px 6px;">
                                            <?php echo $item['is_visible'] ? '👁' : '🚫'; ?>
                                        </button>
                                    </td>
                                    <td style="text-align:center;">
                                        <button type="button"
                                            data-name="<?php echo addslashes(htmlspecialchars($item['name'])); ?>"
                                            class="bulk-delete-btn"
                                            style="background:none;border:none;cursor:pointer;font-size:1.2rem;color:#cc0000;padding:2px 6px;" title="Eliminar producto">✕</button>
                                        <input type="checkbox" class="bulk-delete-input" name="delete_ids[]" value="<?php echo $item['id']; ?>" style="display:none;">
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
            ...
            document.querySelectorAll('.bulk-delete-btn').forEach(function(btn) {
                btn.addEventListener('click', function() {
                    var name = btn.dataset.name || 'este producto';
                    if (!confirm('¿Eliminar "' + name + '"?\nEsta acción no se puede deshacer.')) return;
                    var row = btn.closest('tr');
                    var chk = row.querySelector('.bulk-delete-input');
                    chk.disabled = false;
                    chk.checked  = true;
            ...
            // ── Botón ojito de visibilidad (bulk) ────────────────────────────────────
            document.querySelectorAll('.bulk-vis-btn').forEach(function(btn) {
                btn.addEventListener('click', async function() {
                    ...
                    try {
                        ...
                        if (data.ok) {
                            ...
                            if (data.is_visible) {
                                btn.textContent = '👁';
                                btn.title = 'Visible — clic para ocultar';
                                row.style.opacity = '';
                            } else {
                                btn.textContent = '🚫';
                                btn.title = 'Oculto — clic para mostrar';
                                row.style.opacity = '0.5';
                            }
                        } else {
                            alert('Error al cambiar visibilidad: ' + (data.error || ''));
                        }
                    } catch(e) {
            ...
            document.querySelectorAll('.bulk-sortable').forEach(function(tbody) {
                Sortable.create(tbody, {
                    handle: '.bulk-drag-handle',
                    animation: 150,
                    ghostClass: 'sortable-ghost',
                    onEnd: async function(evt) {
                        var rows  = Array.from(evt.to.querySelectorAll('tr[data-id]'));
                        ...
                        try {
                            var res  = await fetch('editor.php', { method:'POST', body: fd });
                            var data = await res.json();
                            if (!data.ok) alert('Error al guardar orden: ' + (data.error || ''));
                        } catch(e) {
    ...
    <script>
        ...
        function validateItemForm(form) {
            ...
            if (imageUrl && imageUrl !== existingImage && !/^(https?:\/\/)/i.test(imageUrl)) {
